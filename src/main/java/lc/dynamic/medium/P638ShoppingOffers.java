 ...
 * respectively. In special offer 1, you can pay $5 for 3A and 0B. In special
 * offer 2, you can pay $10 for 1A and 2B. You need to buy 3A and 2B, so you may
 * pay $10 for 1A and 2B (special offer #2), and $4 for 2A.
 * 
 * Example 2:
 * 
 ...
 * Output: 11
 * 
 * Explanation: The price of A is $2, and $3 for B, $4 for C. You may pay $4 for
 * 1A and 1B, and $9 for 2A ,2B and 1C. You need to buy 1A ,2B and 1C, so you
 * may pay $4 for 1A and 1B (special offer #1), and $3 for 1B, $4 for 1C. You
 * cannot add more items, though only $9 for 2A ,2B and 1C.
 * 
 * Note:
 * 
 * There are at most 6 kinds of items, 100 special offers. For each item, you
 * need to buy at most 6 of them. You are not allowed to buy more items than you
...
public class P638ShoppingOffers {
	...
	public int shopping(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
		...
		for (List<Integer> offer : special) {
			...
			if (i == needs.size())
				cost = Math.min(cost, offer.get(i) + shopping(price, special, clone));
		}
		return cost;
	}

	static int dotProduct(List<Integer> a, List<Integer> b) {
		int sum = 0;
	...
	public static void main(String[] args) {
		P638ShoppingOffers p = new P638ShoppingOffers();
		...
		Assert.assertEquals(14, p.shoppingOffers(price, special, needs));

		price = Arrays.asList(new Integer[] { 2, 3, 4 });
		special.clear();
		special.add(Arrays.asList(new Integer[] { 1, 1, 0, 4 }));
		special.add(Arrays.asList(new Integer[] { 2, 2, 1, 9 }));
		needs = Arrays.asList(new Integer[] { 1, 2, 1 });

		Assert.assertEquals(11, p.shoppingOffers(price, special, needs));
	}
